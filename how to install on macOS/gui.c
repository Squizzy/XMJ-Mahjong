/* $Header: /home/jcb/MahJong/newmj/RCS/gui.c,v 12.29 2020/05/23 10:07:21 jcb Exp $
 * gui.c
...
static void playerdisp_popup_claim_window(PlayerDisp *pd, const char *lab,int thinking);
...
int rotatelabels = 1; /* rotate the player info labels on the table */
int thinking_claim = 0; /* show ... before players have noclaimed */
int alert_mahjong = 0; /* pop up notice when can mahjong */
...
# endif
  ...
  if ( m->type == CMsgRedirect ) {
    ...
    sprintf(buf,"Server has redirected us to %.220s",address);
  ...
  */
  ...
  case CMsgPlayerDoesntClaim:
    /* there may be a thinking window up, so hide it */
    {
      PlayerP p;
      p = game_id_to_player(the_game,id);
      if ( p == NULL ) {
	...
	if ( p->userdata == NULL ) {
	  ...
	  PlayerDisp *pd = (PlayerDisp *)p->userdata;
	  if ( pd->thinkbox )
	    gtk_widget_hide(pd->thinkbox);
	}
      }
    }
    break;
...
void setup_dialogs(void) {
  ...
  /* check whether to pop down the claim windows */
  {
    int i;
    for (i=0;i<4;i++) {
      if ( ! pdisps[i].claimw || (! GTK_WIDGET_VISIBLE(pdisps[i].claimw) && ! GTK_WIDGET_VISIBLE(pdisps[i].thinkbox) ) ) continue;
...
static void playerdisp_update(PlayerDisp *pd, CMsgUnion *m) {
  ...
  if ( m->type == CMsgPlayerDraws || m->type == CMsgPlayerDrawsLoose) {
    ...
    if ( p == our_player && alert_mahjong
	 && player_can_mah_jong(our_player,HiddenTile,
				game_get_option_value(the_game,GOSevenPairs,NULL).optbool) ) {
      playerdisp_popup_claim_window(&pdisps[0],"Mah-Jong possible!",0);
    }
    numanims++;
  ...
  if ( m->type == CMsgPlayerClaimsPung 
    ...
    case CMsgPlayerDiscards: 
      lab =  m->playerdiscards.calling ? "Calling!" : NULL; break;
    default: assert(0);
    }
    updated = 1;
    if ( lab ) { playerdisp_popup_claim_window(pd,lab,0); }
    if ( thinking_claim && server_pversion < 1111 ) {
      static int warned = 0;
      if ( ! warned ) {
	warned = 1;
	warn("`Thinking' cannot be shown with this server.");
      }
    }
    if ( m->type ==  CMsgPlayerDiscards 
	 && thinking_claim
	 /* before 1111, the server did not send doesntclaims to
	    all players */
	 && server_pversion >= 1111
	 ) {
      // put up thinking labels for the other players
      for (int i=0; i < 4;i++) {
	PlayerDisp *pd = &pdisps[i];
	if ( pd->player->id == our_id ) continue;
	if ( m->playerdiscards.id == pd->player->id ) continue;
	playerdisp_popup_claim_window(pd,"...",1);
      }
    }
    if ( m->type == CMsgPlayerDiscards
	 && m->playerdiscards.id != our_id && alert_mahjong
	 && player_can_mah_jong(our_player,m->playerdiscards.tile,
				game_get_option_value(the_game,GOSevenPairs,NULL).optbool) ) {
      playerdisp_popup_claim_window(&pdisps[0],"Mah-Jong possible!",0);
    }
  } /* end of the claims */
...
static void playerdisp_popup_claim_window(PlayerDisp *pd, const char *lab, int thinking) {
	...
	if ( big_font ) 
      ...
      // now do the same for thinkbox - we no longer support gtk1
      box = pd->thinkbox = gtk_hbox_new(0,0);
      gtk_widget_show(box);
      gtk_widget_set_name(box,"think");
      GtkWidget *tlab = gtk_label_new("...");
      gtk_widget_show(tlab);
      gtk_widget_set_name(tlab,"think");
      gtk_box_pack_start(GTK_BOX(box),tlab,0,0,0);
      vlazy_fixed_put(VLAZY_FIXED(boardfixed),box,0,0);
    }


    GtkWidget *thisw = thinking ? pd->thinkbox : pd->claimw;
    /* set the label */
    if ( ! thinking ) gtk_label_set_text(GTK_LABEL(pd->claimlab),lab);
    ...
    gtk_widget_size_request(thisw,&r);
    switch ( cori ) {
    case 0:
      x = w/2 - r.width/2;
    ...
    vlazy_fixed_move(VLAZY_FIXED(boardfixed),thisw,x,y);

    /* if not already popped up */
    if ( !GTK_WIDGET_VISIBLE(thisw) ) {
      gtk_widget_show(thisw);
    }
}
...
static gint check_claim_window(gpointer data) {
  ...
  if ( !the_game || pd->claim_serial < the_game->serial
...
#   endif
  ...
  while ( rcf && ! feof(rcf) ) {
	...
	} else if ( strncmp(inptr,"RotateLabels",12) == 0 ) {
	  inptr += 12;
	  sscanf(inptr,"%d",&rotatelabels);
	} else if ( strncmp(inptr,"ThinkingClaim",13) == 0 ) {
	  inptr += 13;
	  sscanf(inptr,"%d",&thinking_claim);
	} else if ( strncmp(inptr,"WarnMahjong",11) == 0 ) {
	  inptr += 11;
	  sscanf(inptr,"%d",&alert_mahjong);
  ...
  if ( update_groups & XmjrcDisplay ) {
    ...
    fprintf(ncf,"Display RotateLabels %d%s",rotatelabels,nl);
    fprintf(ncf,"Display ThinkingClaim %d%s",thinking_claim,nl);
    fprintf(ncf,"Display WarnMahjong %d%s",alert_mahjong,nl);
...
#     endif
  ...
  */
  ...
  if ( showwall ) {
    ...
    gtk_widget_show(l);
    if ( rotatelabels ) gtk_label_set_angle(GTK_LABEL(l),270);
    gtk_box_pack_start(GTK_BOX(tmpw),l,1,1,0);
  }
#endif
  ...
  if ( showwall ) {
    ...
    gtk_widget_show(l);
    if ( rotatelabels ) gtk_label_set_angle(GTK_LABEL(l),90);
    gtk_box_pack_start(GTK_BOX(tmpw),l,1,1,0);
  }
#endif
